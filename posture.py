...
def process (input_image, params, model_params):
    ...
    printTick(1) #prints time required till now.
...
def checkPosition(all_peaks):
    ...
    try:
        ...
        if (f):
            # flip the angle since the meaning has changed
            degrees = 180 - degrees

        if (degrees< 90 - tolerance):
            print("Hunchback")
        elif (degrees > 90 + tolerance):
            print("Reclined")
        else:
            print("Straight")
...
def printTick(i): #Time calculation to keep a trackm of progress
    toc = time.time()
    print ('processing time%d is %.5f' % (i,toc - tic))        

if __name__ == '__main__': #main function of the program

    parser = argparse.ArgumentParser(description='Posture recognition')
    parser.add_argument('-r','--realtime NUM', dest="device", default=0, type=int, metavar='',help='use input from video device (default: 0)')
    parser.add_argument('-i','--image PATH', dest="image", type=str, metavar='',help='use an image')
    parser.parse_args()
    ...
    if args.image:
        print("input from image {}".format(args.image))
        if not os.path.isfile(args.image):
            raise OSError("image {} doesn't exist".format(args.image))
        params, model_params = config_reader()
        canvas = cv2.imread(args.image)
        process(canvas, params, model_params)
        showimage(canvas)
    else:
        print("opening video device with index {}".format(args.device))
        cap = cv2.VideoCapture(args.device)
        if not cap.isOpened():
            raise OSError("unable to open camera")

        cap.set(cv2.CAP_PROP_OPENNI_OUTPUT_MODE, 120)
        cap.set(cv2.CAP_PROP_GSTREAMER_QUEUE_LENGTH, 120)
        ...
        while running:
            ...
            ret, frame = cap.read()
            params, model_params = config_reader()
            process(frame, params, model_params)    
            cv2.imshow("capture", frame) 
            if cv2.waitKey(1) in [ord('q'),27,32]: # q or ESC or SPACE
                running = False
        cv2.destroyAllWindows()
        cap.release()
